...
function isLegacyVariantArtifact(filePath) {
    return /(^|-)offline\.|(^|-)online\./.test(basename(filePath));
}
...
function walk(dir) {
    ...
    return readdirSync(dir).flatMap((entry) => {
        ...
        return stats.isFile() && hasArtifactExtension(fullPath) && !isLegacyVariantArtifact(fullPath) ? [fullPath] : [];
    });
}
...
function cleanNativeCollectedArtifacts() {
    for (const entry of readdirSync(outputDir)) {
        if (
            /^app-debug\.apk$/.test(entry)
            || /^app-debug\.aab$/.test(entry)
            || /^app-debug\.ap_$/.test(entry)
            ||
            /^app-release(-unsigned)?\.(apk|aab)$/.test(entry)
            || /^App\.ipa$/.test(entry)
            || new RegExp(`^TokyoRailMap-${version}-(android|ios)(-(offline|online))?\\.(apk|aab|ipa)$`).test(entry)
            || new RegExp(`^TokyoRailMap-${version}-.+-(offline|online)\\.`).test(entry)
        ) {
...
const candidates = Array.from(new Set([
    ...
    ...walk(join(rootDir, 'android/app/build/outputs/apk')),
    ...walk(join(rootDir, 'android/app/build/outputs/bundle')),
