import assert from 'node:assert/strict';
import { readFileSync } from 'node:fs';

class TestElement {
    constructor(tagName) {
        this.tagName = tagName.toLowerCase();
        this.className = '';
        this.textContent = '';
        this.dataset = {};
        this.style = {};
        this.children = [];
    }

    appendChild(child) {
        this.children.push(child);
        return child;
    }

    querySelector(selector) {
        if (!selector.startsWith('.')) return null;
        const className = selector.slice(1);
        const queue = [...this.children];
        while (queue.length) {
            const child = queue.shift();
            if (String(child.className).split(/\s+/).includes(className)) return child;
            queue.push(...(child.children || []));
        }
        return null;
    }

    get outerHTML() {
        const attrs = [];
        if (this.className) attrs.push(`class="${this.className}"`);
        for (const [key, value] of Object.entries(this.dataset)) {
            attrs.push(`data-${key.replace(/[A-Z]/g, (m) => `-${m.toLowerCase()}`)}="${value}"`);
        }
        const content = this.children.length
            ? this.children.map((child) => child.outerHTML).join('')
            : this.textContent;
        return `<${this.tagName}${attrs.length ? ` ${attrs.join(' ')}` : ''}>${content}</${this.tagName}>`;
    }
}

globalThis.HTMLElement = TestElement;
globalThis.MutationObserver = class {
    observe() {}
};
globalThis.document = {
    createElement: (tagName) => new TestElement(tagName),
    documentElement: {
        getAttribute: () => ''
    },
    querySelectorAll: () => []
};

const {
    renderPanelTripDetailStationCellHtml,
    renderPanelTripDetailStopRowHtml
} = await import('../src/features/panel/panelTripDetailStationRenderer.js');

const html = renderPanelTripDetailStationCellHtml({
    className: 'panel-trip-detail-station',
    dataStationId: 'station-1',
    lineColor: '#ff6600',
    stationCode: 'SI11',
    stationName: 'Shinjuku Very Long Station',
    stationId: 'station-1'
});

assert.match(html, /class="panel-trip-detail-station"/);
assert.match(html, /data-station-id="station-1"/);
assert.match(html, /rw-station-code-badge/);
assert.match(html, /rw-station-code-badge-prefix[^>]*>SI</);
assert.match(html, /rw-station-code-badge-suffix[^>]*>11</);
assert.match(html, /panel-trip-detail-station-marquee/);
assert.match(html, /panel-trip-detail-station-name/);
assert.match(html, /Shinjuku Very Long Station/);
assert.doesNotMatch(html, /SI11 Shinjuku Very Long Station/);

const stopRowHtml = renderPanelTripDetailStopRowHtml({
    rowClass: 'panel-trip-detail-row',
    stationClass: 'panel-trip-detail-station',
    timeCellClass: 'panel-trip-detail-time panel-trip-detail-moment',
    timeHtml: '<span class="panel-trip-detail-time-main panel-time-arrive">10:05</span><span class="panel-trip-detail-time-extra is-origin">始发</span>',
    stationId: 'station-1',
    stationName: 'Shinjuku'
});

assert.match(stopRowHtml, /panel-trip-detail-time panel-trip-detail-moment/);
assert.match(stopRowHtml, /panel-trip-detail-time-main panel-time-arrive/);
assert.match(stopRowHtml, /panel-trip-detail-time-extra is-origin/);
assert.doesNotMatch(stopRowHtml, /panel-trip-detail-arrive/);
assert.doesNotMatch(stopRowHtml, /panel-trip-detail-depart/);

const panelSource = readFileSync('src/features/panel/panel.js', 'utf8');
const appCssSource = readFileSync('src/styles/app.css', 'utf8');

assert.match(panelSource, /scheduleMarqueeApply\(tripDetailRoot\)/);
assert.match(panelSource, /html\.\$\{EXPORT_CLASS\} \.panel-trip-detail \{/);
assert.match(panelSource, /width: max-content !important;/);
assert.doesNotMatch(panelSource, /html\.\$\{EXPORT_CLASS\} \.panel-trip-detail-row[^{}]*width: max-content !important;/);
assert.doesNotMatch(panelSource, /flex: 0 0 auto !important/);
assert.match(panelSource, /<div class="panel-trip-detail-time panel-trip-detail-moment">时刻<\/div>/);
assert.match(panelSource, /grid-column:\$\{colStart\} \/ span 2;">时刻<\/div>/);
assert.match(panelSource, /text: '始发'/);
assert.match(panelSource, /text: '终到'/);
assert.match(panelSource, /text: `\+\$\{dwellMinutes\}'`/);
assert.doesNotMatch(panelSource, />到站时间<\/div>/);
assert.doesNotMatch(panelSource, />发车时间<\/div>/);
assert.match(appCssSource, /\.panel-trip-detail-time-main \{/);
assert.match(appCssSource, /\.panel-trip-detail-time-extra \{/);
assert.match(appCssSource, /min-width: 62px;/);
assert.match(appCssSource, /width: 34px;/);
assert.match(appCssSource, /gap: 1px;/);
assert.match(appCssSource, /max-width: min\(300px, calc\(100vw - 40px\)\);/);
assert.match(appCssSource, /\.panel-trip-detail-row\.is-past \.rw-station-code-badge/);
assert.match(appCssSource, /\.panel-trip-detail-row\.is-past \.rw-station-code-badge-prefix/);
assert.match(appCssSource, /\.panel-trip-detail-row\.is-past \.rw-station-code-badge-suffix/);

console.log('panel trip detail station badge marquee smoke ok');
