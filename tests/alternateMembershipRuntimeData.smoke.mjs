...
const { stationsGeoJSON, stationGroups, alternateLineMembership } = await loadRailGeoDataFromDataFolder();
...
const seijogakuenGroup = (Array.isArray(stationGroups) ? stationGroups : [])
    .find((group) => JSON.stringify(group).includes('Odakyu.Odawara.SeijogakuenMae'));
assert.deepEqual(
    Array.isArray(seijogakuenGroup) ? seijogakuenGroup.flat(Infinity) : [],
    ['Odakyu.Odawara.SeijogakuenMae'],
    'alternate source station should not re-enter transfer capsule station groups'
);
