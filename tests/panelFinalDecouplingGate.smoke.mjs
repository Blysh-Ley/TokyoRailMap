...
const appSource = readSourceFile('src/app.js');
...
assert.equal(
    /getAlternateTripSources|alternateTripSourceIndex/.test(panelSource),
    false,
    'ordinary panel timetables must not merge alternate source trips into direction groups'
);
assert.equal(
    /getAlternateTripSources|alternateTripSourceIndex/.test(appSource),
    false,
    'app must not inject alternate source trip providers into ordinary panel timetables'
);
