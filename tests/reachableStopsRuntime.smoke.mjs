...

{
    ...
    const runtime = createTravelSearchMapRuntime({
        ...
        createJourneyPickPinElement: async ({ label, type }) => ({ label, type }),
    ...
    assert.deepEqual(pinStationChanges.at(-1), { origin: 'S1', destination: '', waypoints: {} });

    stationCoords.set('S1', [139.8, 35.7]);
    runtime.syncJourneyPickPinsToStations();
    assert.deepEqual(markers[0].lngLat, [139.8, 35.7]);

    stationCoords.set('S2', [139.9, 35.8]);
    await runtime.showJourneyPickPin({ label: '途径点1', stationId: 'S2', type: 'waypoint-result-1' });
    assert.deepEqual(markers[1].lngLat, [139.9, 35.8]);
    assert.deepEqual(pinStationChanges.at(-1), {
        origin: 'S1',
        destination: '',
        waypoints: { 'waypoint-result-1': 'S2' }
    });
    stationCoords.set('S2', [139.95, 35.85]);
    runtime.syncJourneyPickPinsToStations();
    assert.deepEqual(markers[1].lngLat, [139.95, 35.85]);

    await runtime.showJourneyPickPin({ label: '途径点2', stationId: 'S2', type: 'waypoint-result-2' });
    assert.deepEqual(markers[2].lngLat, [139.95, 35.85]);
    assert.equal(markers[2].element.label, '途径点2');
    assert.equal(removed.includes(markers[1]), true);
    assert.deepEqual(pinStationChanges.at(-1), {
        origin: 'S1',
        destination: '',
        waypoints: { 'waypoint-result-2': 'S2' }
    });

    await runtime.showJourneyPickPin({ lngLat: { lng: 140, lat: 36 }, type: 'origin' });
    assert.equal(removed.includes(markers[0]), true);
    assert.deepEqual(markers[3].lngLat, [140, 36]);
    assert.deepEqual(pinStationChanges.at(-1), {
        origin: '',
        destination: '',
        waypoints: { 'waypoint-result-2': 'S2' }
