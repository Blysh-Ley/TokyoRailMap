...
assert.equal(
    ...
    'min(88vh, calc(100vh - var(--mobile-expanded-sheet-top-clearance)))'
);
...
assert.equal(shell.beginMobileDrag(), true);
assert.equal(shell.updateMobileDrag(-120), true);
assert.equal(shell.endMobileDrag(-120), 'half');
assert.equal(shell.getMobileState(), 'half');
...
assert.match(cssSource, /--mobile-expanded-sheet-top-clearance:\s*calc\(env\(safe-area-inset-top,\s*0px\) \+ 12px\)/);
assert.match(cssSource, /data-native-platform='ios'[\s\S]*--mobile-expanded-sheet-top-clearance:\s*calc\(env\(safe-area-inset-top,\s*0px\) \+ 76px\)/);
assert.match(cssSource, /data-native-platform='ios'[\s\S]*\[data-panel-root\]\[data-panel-presentation='mobile'\][\s\S]*padding-bottom:\s*0 !important/);
assert.match(
    cssSource,
