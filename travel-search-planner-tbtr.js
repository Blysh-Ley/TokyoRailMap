...
const relaxChainFromBoardRaptor = ({ chainTrips, throughRootTripId, startSegmentIndex, startBoardIndex, startBoardStopId, startBoardDepMs, serviceDayStartMs, roundArr, roundParent, improvedStops }) => {
    let boardIndex = Number.isFinite(startBoardIndex) ? Number(startBoardIndex) : 0;
    let boardStopId = normalizeText(startBoardStopId || '');
    let boardDepMs = Number.isFinite(startBoardDepMs) ? Number(startBoardDepMs) : null;

    for (let seg = startSegmentIndex; seg < chainTrips.length; seg += 1) {
        const trip = chainTrips[seg];
        if (!trip) continue;

        if (seg > startSegmentIndex) {
            const first = Array.isArray(trip?.stops) ? trip.stops[0] : null;
            boardIndex = 0;
            boardStopId = normalizeText(first?.stopId || '');
            boardDepMs = Number.isFinite(first?.depMin) ? serviceDayStartMs + first.depMin * 60000 : null;
        }

        if (!boardStopId || !Number.isFinite(boardDepMs)) continue;

        rideTripFromBoardRaptor({
            trip: {
                ...trip,
                throughRootTripId,
                isThroughContinuation: seg > startSegmentIndex,
...
export const collectJourneyCandidatesTBTR = async ({ sourceStops, destinationStops, serviceDay, baseDepartureMs }) => {
...
export async function findPath(startStopId, endStopId, startTime) {
    ...
    const plans = await collectJourneyCandidatesTBTR({
        sourceStops,
        destinationStops,
        serviceDay,
