    ...
    try {
        ...
        if (result.response === 0) {
            ...
            if (process.platform === 'darwin') {
                ...
                const dmgName = `TokyoRailMap-${actualNewVersion}-mac-arm64.dmg`;
...
ipcMain.handle('tokyorail:read-local-file', async (_event, rawInput) => {
    ...
    try {
        ...
        return { status: 200, statusText: 'OK', headers: [['content-type', getContentType(resolved)]], bodyBase64: toBase64(buf) };
    } catch (err) {
